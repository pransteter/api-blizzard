...
use Apis\ControllerFactory\ControllerFactory;

require_once SERVER_ROOT.'core/bootstrap.php';

class Loader
{
    function __construct($permited_urls , $routes , $smarty_object)
    {
		try
		{
            ...
            new ControllerFactory($load_information , $smarty_object);
		}
		catch(\Exception $e)
		{
			exit($e->getMessage());
	...
	private function urlValidation($permited_urls , $routes)
	{
		...
		// comentar a linha abaixo quando estiver em producao
		//if(isset($fragmented_url[1])) { unset($fragmented_url[1]); }
		...
		if($flag_denied_url)
		{
			throw new \Exception('Não existe permissão configurada para este url. Por favor, realize a configuração no arquivo "/core/bootstrap.php".');
		}
		
		if(!isset($routes[$attempt]['module']) || !isset($routes[$attempt]['controller']) || !isset($routes[$attempt]['parameters_limit']))
		{
			throw new \Exception('Não existe rota configurada para este url. Por favor, realize a configuração no arquivo "/core/bootstrap.php".');
		}
